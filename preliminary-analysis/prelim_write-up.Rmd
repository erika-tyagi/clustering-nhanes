...
With this dataset, we then began our preliminary analysis. Specifically, we began exploring our dataset and the variables that we originally chose to include leveragine the EDA techniques learned in class. We also explored the clusterability of our data using hypothesis testing (using Hopkins sparse sampling) and visually (using visual assessment of tendency / ordered dissimilarity images). These led to us believe that our data were likely clusterable (likely with two clusters), although these groups may not be incredibly distinct and of high-quality.

We also began applying clustering techniques. Specifically, we ran a k-means algorithm with two clusters and summarized this fit visually. We also used internal validation techniques to evaluate this fit. Looking at three evalation metrics (Connectivity, Dunn, and Silhouette), the k-means fit was best with 2-3 clusters (when considering anywhere from 2 to 10 potential clusters).   

The visuals summarizing our preliminary analysis are contained in the `preliminary-analysis/images` folder in our repository. This includes a scatterplot matrix of the features that we chose to include related to nutritional outcomes and food security, our ordered dissimilarity image, our cluster plot (leveraging the `fviz_cluster` package's partitioning method visualization functionality), and a selection of scatterplots of the original datasets with colors separating the clusters. 

The scatterplots show that the clusters seem to delineate the two groups in a way that highly correlates to the level of saturated fat (i.e., the two groups are indivduals with low vs. high intake of fat). 
...
•	Additionally, we will layer on demographic information (e.g., income level, age, height, weight, geographic location), as well as create markers for different chronic illnesses (e.g. diabetes, heart disease), to better understand the composition of the clusters. For example, does a cluster with higher prevalence of diabetes emerge that has a certain nutritional profile?

•	Our initial data exploration has suggested we need to explore thoughtfully removing certain outliers (e.g. variables coded as 99999 which are likely some sort of incomplete rather than an intended response). We drop some of these in our preliminary analysis here, but we intend to do so in a more systemic and thoughtful way over the broader data set.
